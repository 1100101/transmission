...
void torrentInit(tr_torrent* tor, tr_ctor const* ctor)
{
    ...
    */
    ...
    tr_logAddInfoTor(tor, fmt::format(_("Torrent download dir: '{path}'"), fmt::arg("path", tor->download_dir().c_str())));
    if (strcmp(tor->download_dir().c_str(), tr_sessionGetDownloadDir(tor->session)) == 0)
    {
        ...
        tr_logAddInfoTor(tor,
                         ...
                         fmt::arg("path", tor->download_dir().c_str())));
    }
    else
    {
        tor->incomplete_dir_ = nullptr;
...
tr_stat const* tr_torrentStat(tr_torrent* tor)
{
    ...
    auto const piece_download_speed_bytes_per_second = tor->bandwidth_.get_piece_speed_bytes_per_second(now, TR_DOWN);
